...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void createDb() {
        ...
        try (InputStream appDatbase = context.getAssets().open(DB_NAME)) {

            //Путь к БД во внешнем хранилище
            String outFileName = DB_PATH;

            // Создать файл во внешнем хранилище
            try (OutputStream newDatabase = new FileOutputStream(outFileName)) {

                // побайтово копируем данные
                byte[] buffer = new byte[1024];
                int length;

                while ((length = appDatbase.read(buffer)) > 0) {
